...
**One real Postgres-specific migration bug caught before it shipped:**
`Hypothesis.direction_if_yes` reuses the existing `PredictionDirection`
enum, and a fresh Alembic-autogenerated `sa.Enum(name='predictiondirection')`
in a *new* migration defaults to `create_type=True` -- which would have
tried to `CREATE TYPE predictiondirection` again on Postgres (already
created by the original `add_prediction_table` migration) and crashed on
`alembic upgrade head` in production, and on downgrade would have tried
to `DROP TYPE` a type `prediction.direction` still depends on. SQLite has
no native enum type, so the full local test suite passed either way --
this only bites in production Postgres. Fixed with `create_type=False`
on that one column; verified with a real upgrade/downgrade/upgrade cycle.
